...
/**
 * BIG NOTE:  This is a customized copy of Drupal\media_migration\Plugin\migrate\process\MediaWysiwygFilter.php.
 *
 ...
 * process:
 *   bar:
 *     plugin: custom_media_wysiwyg_filter
 *     view_mode_matching:
 *       default: full
 *     media_migrations:
 ...
 *   id = "custom_media_wysiwyg_filter"
 * )
 */
class CustomMediaWysiwygFilter extends EmbedFilterBase implements ConfigurableInterface {
  ...
  /**
   * Constructs a new CustomMediaWysiwygFilter instance.
   *
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    $text = preg_replace_callback($pattern, function ($matches) use ($decoder, $entity_type_id, $source_identifier) {
      ...
      try {
        ...
        // Try to map the width to a viewmode other than the default.
        ...
        $setEmbedWidth = true;
        if (!empty($embed_metadata['data-align'])) {
          if (property_exists($this->migration, 'media_view_mode_aligned_size') && isset($this->migration->media_view_mode_aligned_size)) {
            if (empty($source_attributes['width'])
              || (int) $source_attributes['width'] > (int) $this->migration->media_view_mode_aligned_size['max_width']) {
                // Set the viewmode from the aligned size max.
                // This overrides the default from view_mode_matching,
                // and has precedence over media_view_mode_width_ranges below.
                $embed_metadata['view_mode'] = $this->migration->media_view_mode_aligned_size['view_mode'];
                $setEmbedWidth = false;
            }
          }
        }
        // If media_view_mode_width_ranges are configured then map the width to the viewmode.
        // Do this only if media_view_mode_aligned_size didn't set the viewmode.
        if ($setEmbedWidth && !empty($source_attributes['width'])) {
        ...
        // If a viewmode wasn't found, then include the width in metadata.
        if ($setEmbedWidth && !empty($source_attributes['width'])) {
          $embed_metadata['width'] = $source_attributes['width'];
        }
  ...
  protected function getEmbedCode(array $embed_metadata) {
    ...
    // Add style attribute for width if it's in there.  Don't really need the height (that can be automatic).
    if (!empty($embed_metadata['width'])) {
      $attributes['style'] = 'width:' . $embed_metadata['width'] . 'px;';
    }
