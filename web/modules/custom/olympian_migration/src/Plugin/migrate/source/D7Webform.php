...
class D7Webform extends DrupalSqlBase implements RollbackAwareInterface {
  ...
  public function prepareRow(Row $row) {
    ...
    // $row->setSourceProperty('webform_id', $this->generateWebformAlias($row->getSourceProperty('title')));
...
// $old_alias = $this->getAlias($nid);
// $new_alias = $this->generateWebformAlias($row->getSourceProperty('title'));
// 
// // Set the primary alias for the webform
// $row->setSourceProperty('alias', '/' . $new_alias);
// 
// // If old alias is different, store it as a redirect
// if (!empty($old_alias) && $old_alias !== $new_alias) {
//   $row->setSourceProperty('redirect_alias', '/' . ltrim($old_alias, '/'));
// }


  //   return parent::prepareRow($row);
  // }
protected function generateWebformAlias($title) {
  ...
  /**
   * Get alias
   * (see https://www.calebthorne.com/blog/drupal/2016/07/16/drupal-8-migrate-pathauto)
   */
  ...
  protected function buildFormElements($nid) {
    ...
    foreach ($elements_tree as $element) {
      ...
      if (!empty($element['type']) && is_string($element['type'])) {
        ...
        $this->getModuleHandler()->alter('olympian_migration_d7_webform_element_' . $element['type'], $element_markup, $indent, $element);
      ...
      if (!empty($extra['placeholder'])) {
        $new_element['#placeholder'] = $extra['placeholder'];
      }

      // CSS classes.
      if (!empty($extra['wrapper_classes'])) {
        $new_element['#wrapper_attributes']['class'] = explode(' ', $extra['wrapper_classes']);
      }
      if (!empty($extra['css_classes'])) {
        $new_element['#attributes']['class'] = explode(' ', $extra['css_classes']);
      }
...
/**
 * Delete the old alias for a given node ID if it exists.
 *
 * @param int $nid
 *   The node ID to check for an alias.
 */
// protected function deleteOldAlias(int $nid): void {
...
//     $this->database->delete('url_alias')
//       ->condition('source', 'node/' . $nid)
//       ->execute();
// 
//     // Log the deletion.
//     $this->logger->notice('Deleted alias: {alias} for node {nid}', [
//       'alias' => $query,
//       'nid' => $nid,
//     ]);
//   }
