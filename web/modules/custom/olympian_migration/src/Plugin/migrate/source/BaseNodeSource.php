...
abstract class BaseNodeSource extends Node implements ImportAwareInterface {
  ...
  /**
   * The olympian_migration logger channel.
   *
  ...
  public function query() {
    ...
    $query->leftJoin('variable', 'v', "v.name = CONCAT('shared_content_type_', n.type, '_', n.nid, '_shared_content_enabled')");
    $query->addField('v', 'value', 'shared_content_enabled_raw');
  ...
  public function fields() {
    ...
    $fields['shared_content_enabled'] = $this->t('Flag from D7 variable indicating if the shared content is enabled');
    return $fields;
  }
  ...
  public function prepareRow(Row $row) {
    ...
    // Handle shared_content_enabled flag properly.
    $raw_value = $row->getSourceProperty('shared_content_enabled_raw');
    if (!empty($raw_value)) {
      $unserialized_value = unserialize($raw_value);
      if (is_array($unserialized_value)) {
        $row->setSourceProperty('shared_content_enabled', !empty($unserialized_value) ? 1 : 0);
      }
      elseif ($unserialized_value === "1" || $unserialized_value === 1) {
        $row->setSourceProperty('shared_content_enabled', 1);
      }
      else {
        $row->setSourceProperty('shared_content_enabled', 0);
      }
    }
    else {
      $row->setSourceProperty('shared_content_enabled', 0);
