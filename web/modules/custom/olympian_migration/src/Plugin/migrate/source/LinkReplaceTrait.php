...
trait LinkReplaceTrait {
        ...
        if ($created_year < $no_links_prior) {
          ...
          if (strpos($href, '/node/') === 0 || stristr($href, $site_path . '/node/')) {
            ...
            if ($lookup = $this->manualLookup($nid)) {
              ...
              $this->getLogger('olympian_migration')->info('Replaced internal link @link in article @article.', [
                '@link' => $href,
              ...
              $this->getLogger('olympian_migration')->notice('Unable to replace internal link @link in article @article.', [
                '@link' => $href,
                '@article' => $row->getSourceProperty('title'),
              ]);
            }
  ...
  private function postLinkReplace(string $entity_type, array $field_tables) {
    $this->getLogger('olympian_migration')->notice('Beginning link replace.');
    ...
    foreach ($candidates as $entity_id => $cols) {
      ...
      foreach ($cols as $col) {
        ...
        $this->getLogger('olympian_migration')->info('Checking @link in entity @entity.', [
          '@link' => $field_name,
          '@entity' => $entity_id,
        ]);
          ...
          while ($i >= 0) {
            ...
            if (strpos($href, '/node/') === 0 || stristr($href, $site_path . '/node/')) {
              ...
              if ($lookup = $this->manualLookup($nid)) {
                $link->setAttribute('href', '/node/' . $lookup);
                $link->parentNode->replaceChild($link, $link);
                $this->getLogger('olympian_migration')->info('Replaced internal link from /node/@nid to /node/@link in entity @entity.', [
                  '@nid' => $nid,
                  '@link' => $lookup,
                  '@entity' => $entity_id,
                ]);
                ...
                $this->getLogger('olympian_migration')->notice('Unable to replace internal link @link in entity @entity.', [
                  '@link' => $href,
                  '@entity' => $entity_id,
                ]);
              }
    ...
    foreach ($fields as $field => $columns) {
      ...
      foreach ($candidates as $entity_id => $cols) {
        ...
        foreach ($oopsie_daisies as $id => $links) {
          $this->getLogger('olympian_migration')->notice($this->t('Possible broken links found in node @candidate: @links', [
            '@candidate' => $id,
            '@links' => $links,
          ]));
        }
