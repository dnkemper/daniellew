...
namespace Drupal\olympian_migration\Form;
...
class MigrateSettingsForm extends ConfigFormBase {
  ...
  public function getFormId() {
    return 'olympian_migration_settings';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $config = $this->config('migrate_plus.migration_group.olympian_migration');

    // This ensures that the olympian_migration and default migration group
    // configuration exists. This is necessary because that configuration is
    // ignored and this form needs to save data to the olympian_migration config.
    if ($config->isNew()) {
    ...
    $form['markup'] = [
      '#type' => 'markup',
      '#markup' => $this->t('<p>These settings let you configure Olympian Migrate for use on this site.</p>'),
    ];
    ...
    $form['database']['username'] = [
      ...
      '#default_value' => $config->get('shared_configuration.source.database.username') ? $config->get('shared_configuration.source.database.username') : 'drupal_db_user',
      '#required' => TRUE,
    ];
    ...
    $form['database']['host'] = [
      ...
      '#default_value' => $config->get('shared_configuration.source.database.host') ? $config->get('shared_configuration.source.database.host') : 'd7-vr-temp-db.cwqu12tw5vjb.us-east-1.rds.amazonaws.com',
      '#required' => TRUE,
    ];
    ...
    $form['database']['port'] = [
      ...
      '#default_value' => $config->get('shared_configuration.source.database.port') ? $config->get('shared_configuration.source.database.port') : '3306',
      '#required' => TRUE,
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->config('migrate_plus.migration_group.olympian_migration')
