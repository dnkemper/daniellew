...
$settings["simple_environment_indicator"] = $_ENV['SIMPLE_ENV'] ?? '#177AA3 Local';
...
if (getenv('IS_DDEV_PROJECT') == 'true') {
  ...
  $databases['default']['default'] = [
    ...
    'init_commands' => [
        ...
        'lock_wait_timeout' => 'SET SESSION innodb_lock_wait_timeout = 180',
    ],
  ];
  $databases['migrate']['default'] = [
    'database' => $_ENV['DEFAULT_D7'] ?? 'd7',
    'username' => $_ENV['DB_USER'] ?? 'db',
    'password' => $_ENV['DB_PASS'] ?? 'db',
    'prefix' => '',
    'host' => $_ENV['DB_HOST'] ?? 'db',
...
// Set olympian_migration to TRUE if working on johnmaxwulfing site migration.
$config['config_split.config_split.olympian_migration']['status'] = TRUE;
$settings['media_migration_embed_token_transform_destination_filter_plugin'] = 'media_embed';
$settings['olympian_migration_source_uri_prefix'] = $_ENV['D7_SITE'] ?? '';
$settings['shared_configuration.source.constants.public_file_path'] = $_ENV['D7_SITE_FILES'] ?? '';

// DDEV Sync will change FALSE to TRUE when testing private sites locally (artsciportal and artsciadvising)
$config['config_split.config_split.secure_content']['status'] = FALSE;

// DDEV Sync will change FALSE to TRUE  when syncing artsciportal and artsciadvising to allow webform file uploads.
$config['config_split.config_split.webform_upload']['status'] = FALSE;

// DDEV Sync will change FALSE to TRUE  when syncing johnmaxwulfing site.
$config['config_split.config_split.coin']['status'] = TRUE;
