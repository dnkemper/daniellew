 ...
 */
  ...
  $events_header_image_path = theme_get_setting('events_header_image_path');
  $events_header_file_id = theme_get_setting('events_header_image_upload');
  ...
  // If no events_header_image_path is set, exit early.
  if (empty($events_header_image_path)) {
    echo "No 'events_header_image_path' theme setting found, or it is empty. Exiting.\n";
    return;
  }

  // 2) Replace "public://" in $events_header_image_path with "$custom_source_uri_prefix/"
  if (strpos($events_header_image_path, 'public://') === 0) {
    $filename = substr($events_header_image_path, strlen('public://'));
    // Build the new path
    $events_header_image_path = rtrim($custom_source_uri_prefix, '/') . '/' . ltrim($filename, '/');
    echo "Updated events_header_image_path to: $events_header_image_path\n";
  }
  ...
  if (!$file) {
    $file = File::create([
      'uri' => $events_header_image_path,
      'status' => 0,
    ]);
    // Mark as permanent so it doesn't get garbage-collected.
    $file->setPermanent();
    $file->save();
    echo "Created new File entity for URI: $events_header_image_path (File ID: {$file->id()})\n";
  }
  ...
  // 5) For each content type, update nodes to reference this new media entity.
  $types = ['events_landing_page', 'past_events_landing_page'];
  foreach ($types as $type) {
    ...
    if (empty($nids)) {
      echo "No $type nodes found. Nothing to update.\n";
      continue;
    }
    ...
    echo "Updated 'field_event_header_image' on $type nodes.\n";
  }
  $theme_config = \Drupal::configFactory()->getEditable('olympian9.settings');

  // Set the custom theme setting to a new value.
  $theme_config->set('events_header_image_path', $events_header_image_path);
