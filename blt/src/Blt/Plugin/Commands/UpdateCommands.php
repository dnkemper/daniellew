...
class UpdateCommands extends BltTasks {
  ...
  /**
   * Execute artsci application updates.
   *
   * @command artsci:update
   */
  public function artsciUpdate() {
    ...
    $methods = array_filter($reflection->getMethods(), function ($v) {
      return $v->name != 'artsciUpdate' && $v->class == UpdateCommands::class;
    });
  ...
  protected function getSchemaVersion() {
    $file = $this->getConfigValue('repo.root') . '/blt/.artsci_schema_version';
  ...
  protected function update1001() {
    ...
    foreach ($sites as $site) {
      $yaml = YamlMunge::parseFile("{$root}/web/sites/{$site}/blt.yml");
      $db = $yaml['drupal']['db']['database'];

      $replace = <<<EOD
if (file_exists('/var/www/site-php')) {
  require '/var/www/site-php/artsci/{$db}-settings.inc';
}
...
EOD;
      ...
      $result = $this->taskReplaceInFile("{$root}/web/sites/{$site}/settings.php")
        ->from($search)
        ->to($replace)
        ->run();
  ...
  protected function update1002() {
    ...
    foreach ($sites as $site) {
      $file = "{$root}/web/sites/{$site}/blt.yml";
      $yaml = YamlMunge::parseFile($file);
      $yaml['project']['profile']['name'] = 'artsci';
      $yaml['cm']['core']['dirs']['sync']['path'] = '../config/default';
      ...
      $yaml['sync']['commands'] = [
        ...
        'artsciw:multisite:noop',
      ];

      if (isset($yaml['project']['requester'])) {
        $requester = $yaml['project']['requester'];
        unset($yaml['project']['requester']);
        $yaml['artsci']['artsci']['requester'] = $requester;
      }

      file_put_contents("{$root}/web/sites/{$site}/blt.yml", Yaml::dump($yaml, 10, 2));
    }
...
\$additionalSettingsFiles = [
  DRUPAL_ROOT . "/sites/settings/artsci.settings.php"
];
...
EOD;
    ...
    foreach ($sites as $site) {
      $file = "{$root}/web/sites/{$site}/settings/includes.settings.php";
  ...
  protected function update1005() {
    ...
    foreach ($sites as $site) {
      ...
      $vm_config['apache_vhosts'][] = [
        'servername' => "{$id}.artsci.local.site",
  ...
  protected function update1006() {
    ...
    foreach ($sites as $site) {
      ...
      $id = Multisite::getIdentifier("https://{$site}");
      $yaml = YamlMunge::parseFile($file);
      $yaml['project']['local']['hostname'] = "{$id}.artsci.ddev.site";
      file_put_contents("{$root}/web/sites/{$site}/blt.yml", Yaml::dump($yaml, 10, 2));

      $file = "{$root}/web/sites/{$site}/local.drush.yml";
  ...
  protected function update1013() {
    ...
    foreach ($sites as $site) {
      ...
      foreach (['local', 'dev', 'stage', 'prod'] as $env) {
  ...
  protected function update1016() {
    ...
    foreach ($sites as $host) {
      ...
      $yaml['local']['root'] = '/var/www/html/web';
