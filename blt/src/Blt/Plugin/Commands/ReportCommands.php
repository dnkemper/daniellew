...
class ReportCommands extends BltTasks {
   ...
   *
   * @command artsci:report:analytics
   *
   * @aliases ura
   *
          ...
          if (str_contains($result->getMessage(), 'Drupal Settings File   :  MISSING')
            ...
            if (in_array($machine_name, $this->getD9ApplicationList())) {
              ...
              $result = $this->getDrushTask($debug)
                ...
                ->alias("$machine_name.prod")
                ->ansi(FALSE)
                ->drush('artsci:get:gtm-containers')
                ->option('uri', $domain)
                ->run();
   ...
   *
   * @command artsci:report:last-login
   *
   * @aliases urll
   *
   * @throws \Robo\Exception\TaskException
  ...
  //   $client = $this->getAcquiaCloudApiClient($this->getConfigValue('artsci.credentials.acquia.key'), $this->getConfigValue('artsci.credentials.acquia.secret'));
