...
class GitCommands extends BltTasks {
  ...
  public function preSync(CommandData $commandData) {
    ...
    file_put_contents("{$root}/web/sites/sites.local.php", "<?php\n");
  ...
  public function deploy() {
    ...
    $applications = $this->getConfigValue('artsci.applications');

    $connector = new Connector([
      'key' => $this->getConfigValue('artsci.credentials.aws.key'),
      'secret' => $this->getConfigValue('artsci.credentials.aws.secret'),
    ]);
  ...
  protected function writeGitVersion() {
    ...
    if (!$result->wasSuccessful()) {
      ...
      $files = $finder
        ->files()
        ->in([
          "{$deploy}/web/profiles/custom/",
          "{$deploy}/web/themes/custom/",
          "{$deploy}/web/modules/custom/",
        ])
        ->depth('< 2')
        ->name('*.info.yml')
        ->sortByName();
